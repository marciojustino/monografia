...
Os discos rígidos (\textit{HD's}) são usados para armazenar informações \textit{não voláteis}\footnote{Refere-se a informações que permanecem em memória mesmo depois que a energia é cortada do dispositivo ou o mesmo é desligado. Já a informação volátil é armazenada em memória volátil, como a memória RAM, e que são perdidas quando a energia do dispositivo é cortada ou o mesmo é desligado.} como programas, documentos, planilhas, anexos de e-mail, arquivos de sistema e outros criados pelo usuário. 
...
\begin{figure}[htp]
  ...
  \caption{Disco rígido moderno \cite{WikiHD2013}}
  \label{fig:harddisk}
\end{figure}
...
O sistema de arquivos NTFS foi projetado para melhor confiabilidade, segurança e suporte de grandes dispositivos de armazenamento de dados. Dispõe do uso de estruturas genéricas que servem de envoltório para estruturas de dados com conteúdo específico. Desta forma, o NTFS se torna um projeto escalável pois a estrutura interna de dados pode mudar inúmeras vezes enquanto que a sua casca (a estrutura genérica) permanece constante. Um bom exemplo desse modelo é que todos os bytes\footnote{Termo binário que representa 8 unidades da menor unidade de informação (bit) que pode ser armazenada ou transmitida \cite{WikiBytes2012}.} são alocados em arquivos no sistema \cite{Carrier2005}.
...
Segundo Svensson (2005, p. 26), todos os arquivos no sistema NTFS apresentam ao menos uma entrada no registro da \emph{Master File Table} (MFT)\footnote{Conjunto de registros que são continuamente modificados quando um arquivo ou diretório sofre alteração}. Os primeiros 16 registros da MFT são reservados pelo sistema de arquivos para manter metadados\footnote{Dados que definem ou descrevem outra parte dos dados \cite{Wyman2012}.} específicos do NTFS. A tabela \ref{tab:tab02} detalha os 16 primeiros registros reservados ao NTFS na MFT:
...
\begin{table}[htb]
  ...
  \begin{tabular}{|l|p{10cm}|}
    ...
    \hline
    \$LOGGED\_UTILITY\_STREAM & A EFS (Encrypting File System - Essa tecnologia permite a encriptação de arquivos de forma transparente protegendo informações confidenciais de invasores com acesso físico ao computador \cite{WikiEFS2013}) armazena informações que são usadas na manipulação de arquivos encriptados. \\
    \hline
  \end{tabular}
  \caption{Registros reservados do NTFS na Master File Table.}
...
As entradas de índice consistem do nome do arquivo e da cópia das informações padrões da entrada de diretório. Como o número de arquivos em diretórios pode crescer, o espaço pode não ser suficiente para armazenar todas as entradas de índice então, os índices adicionais são armazenados na área de alocação conhecida como \emph{``Index Allocation Buffer''}. A área de bitmap também é usada para indicar qual VNCs presente no ``Index Allocation Buffer'' estão sendo usados.
