...
Segundo Kloe (2007, p. 8), um arquivo fragmentado é um arquivo dividido em várias partes onde cada parte pode estar localizada em um lugar diferente em um mesmo conjunto de dados. Sistemas operacionais modernos, tais como o NTFS, tentam gravar os dados evitando a fragmentação porém, ainda existem algumas situações em que a fragmentação ocorre.

\begin{itemize}
 \item Quando não há mais espaço de media suficiente continuamente para a gravação de um arquivo, assim para ser alocado no disco ele tem que ser dividido em dois ou mais fragmentos.
 \item Na sequência de um arquivo o espaço restante não é suficiente para a gravação do arquivo sequencialmente, sendo necessária a divisão do arquivo em dois ou mais fragmentos.
\end{itemize}
